# Generate AnswerFile
#  We need to Generate an AnswerFile that can be used to install Proxmox
#  The host reaches out and gives the disk and nic to use
#  We need to use that, then generate the answer file using the servers
#  defaults for timezone, language, keyboard layout, etc.
#  We also need to generate a random password for the root user, that is hashed
import re
...
def get_country(locale_str: str | None) -> str:
...
def gen_ans_file_with(mgmt_nic: str, disk: str) -> str:
    ...
    toml_str = re.sub(r'^"([^"]+)"\s*=', r"\1 =", toml_str, flags=re.MULTILINE)
    return toml_str
