...
from proxmox_auto_installer.constants import (
...
class DiskConfig(BaseModel):
    ...
    def validate_zfs_raid_required(cls, raid_value, values):  # type: ignore
        ...
        if fs == "zfs":
            if raid_value is None:
                raise ValueError("ZFS RAID configuration is required when filesystem is zfs")
    ...
    def validate_btrfs_raid_required(cls, raid_value, values):  # type: ignore
        ...
        if fs == "btrfs":
            if raid_value is None:
                raise ValueError("Btrfs RAID configuration is required when filesystem is btrfs")
    ...
    def validate_disk_list_paths(cls, disk_list_value: list[str], values):  # type: ignore
        ...
        if fs in raid_disk_requirements and raid in raid_disk_requirements[fs]:
            ...
            if len(disk_list_value) < required_disks:
                ...
                raise ValueError(f"At least {required_disks} disks are required for {fs_label} {raid}")
        return disk_list_value
