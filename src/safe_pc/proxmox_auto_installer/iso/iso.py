...
from safe_pc.utm.iso_dl import ISODownloader
...
class ProxmoxISO:
    ...
    async def download(
        self,
        on_update: Callable[[int, int, str], Any] = lambda x, t, y: print(
            f"{x/t}%, {y}"
        ),
    ) -> bool:
        ...
        return await ISODownloader(
            get_latest_prox_url_w_hash, dest_dir=self.iso_dir, on_update=on_update
        )
...
class ModifiedProxmoxISO:
    ...
    def move_iso_to_final_location(self, final_dir: Path | None = None) -> Path:
        ...
        # 1) Resolve source
        src = (
            Path(getattr(self, "modified_iso_path", ""))
            if getattr(self, "modified_iso_path", None)
            else None
        )
        if not src or not src.exists():
            ...
            self.LOGGER.info(
                f"[move_iso_to_final_location] Auto-selected ISO source: {src}"
            )
